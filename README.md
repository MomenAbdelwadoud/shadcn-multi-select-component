...
The component uses a hidden input that takes the values and stringify them, then you can access the string value and parse it into an array

Example with Nextjs Form action:

```tsx
function somFunction(formData: FormData) {
	const frameworks = JSON.parse(formData.get("frameworks"));
